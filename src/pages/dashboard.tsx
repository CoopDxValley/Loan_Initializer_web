...
const BusinessSelection = () => {
  const [loanIntentions, setLoanIntentions] = useState([
    "Growing my business",
    "Growing my business",
    "Growing my business",
    "Growing my business",
  ...
  return (
    <div className="w-full flex flex-col mt-10 sm:flex-row sm:flex-wrap sm:justify-between gap-5 select-none">
      {loanIntentions.map((item, idx) => (
        <div className="h-[100px] sm:w-56 flex items-center justify-between px-4 py-4 gap-4 border-2 sm:relative">
          <img src="/assets/Business-icon.png" className="w-8" alt="" />
          <span>{item}</span>
          {selectedIdx.indexOf(idx) === -1 ? (
            <GoCircle
              className="cursor-pointer sm:absolute sm:top-2 sm:right-2 text-3xl sm:text-xl"
              onClick={() => {
    ...
    <div className="max-w-screen-lg mx-auto">
      <div className="w-full flex flex-col px-6 sm:px-20 gap-8 py-4">
        ...
        <div className="flex flex-col sm:items-center px-6 border-2 pt-6 pb-12 gap-4">
          ...
          <span className="border-2 text-center w-full sm:w-fit py-2 sm:px-16 font-bold text-[#206EEE] text-2xl">
            $70000
          </span>
